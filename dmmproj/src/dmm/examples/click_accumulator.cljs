(ns dmm.examples.click-accumulator
...
(def click-val (atom {:signal {}}))
(defn mouse-pressed-monitor- []
  (go (let [signal ((async/alts! [mouse-pressed-channel] :default {}) 0)]
        (reset! click-val
                {:signal
                 (if (= signal {})
                   {}
...
(defn mouse-pressed [state event]
  ...
  (go (async/>! mouse-pressed-channel event))
  state)

(q/defsketch clicker-exp
  :host "click-accum"
  :size [500 500]
